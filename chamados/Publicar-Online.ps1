<#
  Publicar-Online.ps1  -  APP CHAMADO
  Liga o servidor local (Servir.ps1) + um tunel Cloudflare e mostra uma URL PUBLICA
  para acessar de qualquer lugar (fora da sua rede). O app continua protegido por LOGIN.
...
Write-Host ''
Write-Host '  Iniciando o APP CHAMADO para publicacao...' -ForegroundColor Cyan

# 1) garante o servidor (Servir.ps1) rodando
if (-not (Porta-Ativa)) {
  Start-Process powershell -ArgumentList '-NoProfile', '-WindowStyle', 'Hidden', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $root 'Servir.ps1') -WindowStyle Hidden
  for ($i = 0; $i -lt 30; $i++) { Start-Sleep -Milliseconds 700; if (Porta-Ativa) { break } }
}
if (-not (Porta-Ativa)) { Write-Host '  ERRO: o servidor (porta 8790) nao subiu. Veja o Servir.ps1.' -ForegroundColor Red; pause; exit 1 }
Write-Host '  Servidor OK (porta 8790, com login).' -ForegroundColor Green

# 2) localiza o cloudflared
$cf = (Get-Command cloudflared -ErrorAction SilentlyContinue).Source
if (-not $cf) { $cf = @("$env:LOCALAPPDATA\Microsoft\WinGet\Links\cloudflared.exe") | Where-Object { Test-Path $_ } | Select-Object -First 1 }
...
$log = Join-Path $env:TEMP 'appchamado-tunnel.log'
...
Write-Host '   APP CHAMADO - PUBLICADO ONLINE' -ForegroundColor Green
Write-Host '==================================================================' -ForegroundColor Cyan
if ($url) {
  Write-Host ''
  Write-Host "   URL publica:  $url" -ForegroundColor Yellow
  Write-Host ''
  Write-Host '   Cada pessoa entra com o PROPRIO usuario e senha do APP CHAMADO.' -ForegroundColor White
